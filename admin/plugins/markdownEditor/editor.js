// Markdown editor plugin -- JS API.
//
// Exposes window.lpMarkdownEditor.toolbarHtml() for dynamic JS callers
// (eventList admin.js's renderCard, future modules' admin.js). Returns
// the same DOM the PHP helper markdown_editor_toolbar_html() emits, so
// statically- and dynamically-rendered toolbars are byte-identical and
// the single delegated click handler in config.js works on both.
//
// The toolbar's behavior (button click handlers, preview toggle, mousedown
// selection-preserve) lives in config.js for now -- delegated via
// $(document) so it fires regardless of when/where the toolbar lands.
// Moving those handlers into this plugin is a follow-up cleanup.
...
'</div>';
    ...
    window.lpMarkdownEditor = window.lpMarkdownEditor || {};
    window.lpMarkdownEditor.toolbarHtml = toolbarHtml;

    // Backwards compatibility for existing call sites that referenced the
    // global before this plugin existed. Drop after refactoring callers.
    window.buildMarkdownToolbarHtml = toolbarHtml;
})();
